...
def handle_translate_word(message: types.Message):
    bot.send_message(message.chat.id, "Send me the word you want to translate:")

@bot.message_handler(func=lambda msg: msg.text == "🗣 Learn a Phrase")
def handle_learn_phrase(message: types.Message):
    topics = load_json(PHRASES_FILE)
    if not topics:
        bot.send_message(message.chat.id, "No phrase topics available.")
        return
    markup = types.InlineKeyboardMarkup()
    for topic in topics.keys():
        markup.add(types.InlineKeyboardButton(topic, callback_data=f"phrase_topic:{topic}"))
    bot.send_message(message.chat.id, "Choose a topic:", reply_markup=markup)

@bot.message_handler(func=lambda msg: True)
def handle_word_input(message: types.Message):
    word = message.text.strip()
    info = find_word_info(word)
    if info:
        translation = info.get("translation") or "N/A"
    else:
